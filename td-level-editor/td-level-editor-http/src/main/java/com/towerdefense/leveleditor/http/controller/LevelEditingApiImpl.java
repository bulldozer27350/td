...
import com.towerdefense.leveleditor.http.model.AddTowerTypeRequest;
...
public class LevelEditingApiImpl extends AbstractEditorController implements LevelEditingApi {
    ...
    public LevelEditingApiImpl(
            EditablePathMapper pathMapper,
            EditableAttackMapper attackMapper,
            EditableWaveMapper waveMapper,
            EditableLevelMapper levelMapper) {
    ...
    public ResponseEntity<Void> addAttack(String levelId, @Valid CreateAttackRequest createAttackRequest) {
        ...
        com.towerdefense.editor.api.model.draft.EditableAttack attack = 
            new com.towerdefense.editor.api.model.draft.EditableAttack(createAttackRequest.getId());
    ...
    @Override
    public ResponseEntity<Void> addTowerTypeToLevel(String levelId, @Valid AddTowerTypeRequest addTowerTypeRequest) {
        LevelEditorApi levelEditor = getLevelEditor(levelId);
        if (levelEditor == null) {
            return ResponseEntity.notFound().build();
        }

        boolean success = levelEditor.addTowerType(addTowerTypeRequest.getTowerTypeId());
    ...
    public ResponseEntity<Void> addWave(String levelId, String attackId, @Valid EditableWave editableWave) {
        ...
        try {
            ...
            boolean success = attackEditor.addEditableWave(
                domainWave.getId(),
                domainWave.getStartTick(),
                domainWave.getSpawnInterval(),
                domainWave.getCount(),
                domainWave.getEnemyType(),
    ...
    public ResponseEntity<List<EditableAttack>> getLevelAttacks(String levelId) {
        ...
        List<EditableAttack> attacks = levelEditor.getAllAttackEditors()
            .stream()
            .map(AttackEditorApi::getCurrent)
            .map(attackMapper::toHttp)
            .toList();
    ...
    public ResponseEntity<List<EditablePath>> getLevelPaths(String levelId) {
        ...
        List<EditablePath> paths = levelEditor.getCurrentLevel().getPaths()
            .stream()
            .map(pathMapper::toHttp)
            .toList();
    ...
    @Override
    public ResponseEntity<EditableLevel> updateLevelMetadata(String levelId, @Valid LevelMetadataUpdate levelMetadataUpdate) {
        LevelEditorApi levelEditor = getLevelEditor(levelId);
        if (levelEditor == null) {
            return ResponseEntity.notFound().build();
        }
